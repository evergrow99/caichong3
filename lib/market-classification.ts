...
const CONTEXT_RULES: CategoryRule[] = [
  ...
  {
    category: "视频",
    pattern: /(?:制作|生成|剪辑|交付|完成|做).{0,18}(?:\d+\s*(?:秒|分钟)|短视频|视频|成片)|(?:视频|短片|成片).{0,18}(?:\d+\s*(?:秒|分钟)|制作|生成|剪辑|交付|完成)/i,
    weight: 11
  },
  {
    category: "声音",
    pattern: /(?:背景音乐|配乐|bgm).{0,24}(?:视频|短视频|vlog|up主|科普)|(?:视频|短视频|vlog|up主|科普).{0,24}(?:背景音乐|配乐|bgm)/i,
    weight: 13
  },
  {
    category: "图片",
