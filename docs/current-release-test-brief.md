...
## 小修复待测摘要 2026-06-05 任务详情附件层级与超时红条兜底

给测试线程的简单指令：

```text
请读取 docs/current-release-test-brief.md 的“小修复待测摘要 2026-06-05 任务详情附件层级与超时红条兜底”，按本节范围测试；能修小问题就修，但涉及钱、短信、结算、权限、生产配置的动作先问主人。最后把测试结论写回本节下方的“小修复测试结论”小节。
```

### 测试目标

本轮是刚上线版本之后的小修复，只覆盖任务详情页两个体验点：

1. 发布任务时上传的参考附件，应该跟在需求描述正文下面展示。
2. 任务详情远程读取超时但已有本地任务内容可展示时，不应再残留顶部红色 `读取超时，请点击刷新重试` 错误条。

### 本轮版本状态

- 分支：`main`
- 本轮起点：刚上线提交 `5c0f873 Prepare current release package`。
- 当前工作区有小修复未提交改动。
- 测试范围以当前本地工作区为准。
...
1. 任务详情附件层级调整
- 发布任务时上传的参考附件，从报酬、创建/发布时间下方移动到需求描述正文下方。
- 目标是让 `需求描述 + 附件` 作为同一个需求整体被用户理解。
- 保留原有附件预览、键盘打开、下载按钮和下载阻止冒泡逻辑。

2. 附件块间距调整
- 移动附件位置后，取消附件区自身额外顶部外边距。
- 避免附件列表看起来像脱离需求正文的独立区块。

3. 详情超时红条兜底
- 新增统一的 `DETAIL_FETCH_TIMEOUT_ERROR` 文案常量。
- 使用 `tasksRef` 保留最新任务列表，避免异步请求失败时只拿到旧闭包里的任务列表。
- 当前端详情请求触发 8 秒超时，但任务列表或本地缓存里已有该任务时，继续展示本地任务内容，并清掉顶部红色超时错误条。
...
- `npm run lint` 通过，0 error，13 warning；warning 为既有 `<img>` 与 React Hook dependency 提示。
- `npm run build` 通过。
- 本地开发服务日志核对：任务详情和附件 inline 预览存在慢请求，能解释截图中的“读取超时”红条来源。
- 本地浏览器 console error 检查未见错误。
...
- 确认附件显示在需求描述正文下方。
- 确认报酬和创建/发布时间显示在附件之后。
- 确认附件块没有多余顶部空隙，视觉上仍属于需求正文区域。

长需求描述：
- 打开长需求描述任务。
- 测试 `展开完整需求` / `收起需求`。
- 确认展开、收起时附件位置不跳乱、不遮挡、不和元信息混在一起。

多附件：
- 打开含多个参考附件的任务。
- 确认附件换行、文件名省略、文件大小展示正常。
- 点击附件行应打开预览。
- 键盘 Enter / Space 应能打开预览。
- 点击下载按钮应只触发下载，不应同时打开预览。

超时红条兜底：
- 模拟或复现任务详情远程读取较慢的场景。
- 当本地已有任务内容可展示时，页面应继续展示任务内容。
- 顶部不应残留红色 `读取超时，请点击刷新重试` 错误条。
- 如果没有本地任务内容可兜底，仍允许展示真实错误提示。

移动端窄屏：
- 检查附件卡片、文件名、下载按钮不挤压、不遮挡。
- 检查需求描述、附件、报酬、发布时间的阅读顺序清楚。

### 风险和边界

- 本轮不改变接口和权限规则，理论上不应影响订单归属、附件访问或下载权限。
- 如果真实环境才虫详情或附件读取持续慢，用户仍可能在没有本地缓存可用时看到超时提示；本次只修复已有本地内容时的残留红条。
- 附件位置调整可能影响移动端布局，需要重点看窄屏。
...
- 是否发现 bug：否
- 是否需要修 bug 线程：不需要
- 是否需要补测：需要，建议用真实带发布附件的任务详情人工补看一次
- 是否可以交上线总控判断：可以
- 下一步口令：上线判断
- 一句话原因：本地硬检查、只读接口、权限边界、浏览器首页和代码路径核对均通过；但当前本地测试账号没有带发布附件的任务，附件真实详情交互和精确超时场景未完整覆盖。

测试线程名称：Codex 本地小修复测试线程
对应上线摘要：2026-06-05 任务详情附件层级与超时红条兜底
测试环境：本地 main 工作区；Next dev server http://localhost:3000；本地 HTTP/API、内置浏览器冒烟和代码路径核对
测试时间：2026-06-05 17:22 CST

测试范围：
按本节小修复范围验证任务详情附件层级、附件样式间距、详情超时红条兜底、基础构建检查、只读任务详情接口、订单归属权限边界和移动端窄屏首页冒烟。未触发真实付款、采用投稿、结算、退款、真实短信；未修改生产配置。

通过路径：
- `git status --short --branch` 通过：当前分支 `main...origin/main`，未提交文件为 `app/globals.css`、`components/order-console.tsx`、`docs/current-release-test-brief.md`、`docs/release-management.md`。
- `git diff --check` 通过。
- `npm run lint` 通过，0 error，13 warning；warning 为既有 `<img>` 与 React Hook dependency 提示。
- `npm run build` 通过。
- `npm run test:regression` 通过，输出 `UI regression checks passed.`。
- `/api/health/readiness` 本地只读检查通过，返回 `ready=true`。
- 本地开发登录接口成功登录 `13900000000`；读取任务列表成功，当前账号只有 1 个无附件关闭订单。
- 所属账号读取任务详情 `/api/tasks/08487707-1982-4d25-ba1c-7187c047ac09?includeAttachments=1&sync=0` 成功，返回 `attachments: []`，状态 `CLOSED`。
- 其他开发账号读取同一任务详情返回 404 和 `订单不存在，或你没有权限查看这条订单`，订单归属边界符合预期。
- 内置浏览器打开本地首页成功，页面标题为 `AICHONG`，console error 为 0。
- 内置浏览器 390x844 窄屏首页冒烟成功，任务说明和登录入口可见，console error 为 0。
- 代码路径核对通过：附件块位于需求描述 section 内、报酬和时间元信息之前；附件项仍保留点击预览、Enter/Space 键盘预览、下载按钮 `stopPropagation()`；CSS 已将 `.task-attachment-section` 和内部 compact list 顶部外边距归零。
- 代码路径核对通过：`DETAIL_FETCH_TIMEOUT_ERROR` 统一超时文案；`tasksRef` 保留最新任务列表；详情请求超时且存在 fallback task 时调用 `setError(null)`，没有 fallback task 时仍保留真实错误提示。

发现的问题：
无。本轮测试未发现需要立即修复的业务代码问题。

已修复的问题：
无。本轮测试线程未修改业务代码。

仍未验证：
- 未用真实带发布附件的任务详情验证附件显示在需求描述正文下方、附件多行换行、文件名省略、文件大小展示、点击预览、Enter/Space 打开预览、下载按钮不同时打开预览。
- 未用长需求描述且带附件的真实任务验证 `展开完整需求` / `收起需求` 后附件位置不跳乱、不遮挡。
- 未稳定复现一个真实“详情远程读取正好超过 8 秒但本地任务内容已展示”的浏览器场景；本轮按代码路径、当前慢请求背景和无附件样本接口做验证。
- 未在生产环境、真实用户账号、真机移动端验证。

残留风险：
- 如果真实环境才虫详情或附件读取持续慢，且前端没有任何本地任务内容可兜底，用户仍会看到真实超时提示；这是本轮明确保留的行为。
- 当前本地测试账号没有带发布附件任务，附件真实视觉和交互仍建议上线总控或主人用已有真实附件任务补看。
- 本轮不改接口和权限规则，但附件属于 P1 体验链路，真实附件预览/下载仍应在上线前或上线后重点观察。

是否涉及真实付款/结算/短信：
否。未触发真实付款、真实采用投稿、真实结算、退款或真实短信。

是否改动了代码：
否。本轮测试线程未改业务代码；仅写回本测试结论。

改动文件：
`docs/current-release-test-brief.md`

验证命令和结果：
- `git status --short --branch`：通过，当前仅有本轮 4 个未提交文件。
- `git diff --check`：通过。
- `npm run lint`：通过，0 error / 13 warning。
- `npm run build`：通过。
- `npm run test:regression`：通过。
- `curl -s http://localhost:3000/api/health/readiness`：通过，`ready=true`。
- `curl -s -c /tmp/aichong-smallfix-cookie.txt -X POST http://localhost:3000/api/auth/dev-login ...`：通过，登录 `13900000000`。
- `curl -s -b /tmp/aichong-smallfix-cookie.txt http://localhost:3000/api/tasks?page=1&pageSize=50`：通过，返回 1 个无附件关闭订单。
- `curl -s -b /tmp/aichong-smallfix-cookie.txt http://localhost:3000/api/tasks/08487707-1982-4d25-ba1c-7187c047ac09?includeAttachments=1&sync=0`：通过，返回任务详情且 `attachments: []`。
- `curl -s -b /tmp/aichong-smallfix-137-cookie.txt -w HTTP_STATUS:%{http_code} http://localhost:3000/api/tasks/08487707-1982-4d25-ba1c-7187c047ac09?includeAttachments=1&sync=0`：通过，返回 404，其他账号不可见。
- 内置浏览器桌面首页冒烟：通过，console error 为 0。
- 内置浏览器 390x844 窄屏首页冒烟：通过，console error 为 0。

是否建议上线：建议补测后上线
一句话理由：小修复的构建、基础 UI、只读接口、权限边界和关键代码路径均通过；但真实带附件详情页和精确超时兜底场景受本地数据限制未完整验证，建议补看后交上线总控最终判断。
```

### 小修复上线总控最终判断 2026-06-05

```text
上线结论：可上线，带观察。
判断时间：2026-06-05 17:27 CST

判断依据：
- 主人已明确下达“本轮上线”指令。
- 本轮只修改任务详情附件展示层级和详情超时红条兜底，不新增接口、环境变量、数据库 migration 或生产配置。
- 本轮不改变付款、退款、结算、采用投稿、短信触发规则，也不改变附件访问或下载权限。
- 小修复测试线程未发现需要立即修复的业务代码问题。
- 上线总控复核硬门槛：`git diff --check` 通过，`npm run test:regression` 通过，`npm run lint` 通过且 0 error / 13 warning，`npm run build` 通过。

接受带观察上线的原因：
- 未验证项主要集中在真实带发布附件的任务详情视觉和精确 8 秒超时场景；本轮代码不改变接口和权限边界，风险可通过上线后真实页面观察承接。
- 真实付款、真实采用投稿、结算、真实短信均不属于本轮触发范围，也没有在测试或上线中触发。

上线后观察：
- 真实含发布附件任务详情中，附件是否显示在需求描述正文下方。
- 长需求描述展开/收起后，附件、报酬和发布时间是否不跳乱、不遮挡。
- 多附件任务中，附件预览、键盘打开和下载按钮是否正常。
- 慢详情读取时，如果已有本地任务内容，顶部是否不再残留红色超时错误条。
- 移动端窄屏下附件卡片和下载按钮是否不挤压。
