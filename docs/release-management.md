...
## 上线功能交接摘要 2026-06-05 任务详情附件层级与超时红条兜底
...
本轮起点：
- 本功能线开始前，工作区为 `main...origin/main` 且无未提交文件。
- 本摘要只包含本功能线新增改动，不包含此前“2026-06-05 当前未提交功能包”的旧内容。

准备纳入本轮上线的改动：
- 任务详情页信息层级调整：发布任务的附件从报酬、创建/发布时间下方，移动到需求描述正文下方，使“需求描述 + 附件”作为同一个需求整体展示。
- 附件块间距调整：移动后取消附件列表自身的额外顶部外边距，避免附件看起来像独立区块。
- 任务详情读取超时兜底：当前端详情请求触发 8 秒超时，但任务列表或本地缓存里已有该任务时，继续展示本地任务内容，并清掉顶部“读取超时，请点击刷新重试”的红色错误条。
- 保留原有附件预览、键盘打开、下载按钮和下载阻止冒泡逻辑，不改变附件访问和下载权限。

不纳入本轮的内容：
- 不修改任务详情接口、投稿接口、附件代理接口或才虫同步策略。
- 不修改付款、退款、结算、采用投稿或短信触发规则。
- 不修改任务价格、提交期、选择期、支付超时等业务规则。
- 不新增后台统计、生产配置、环境变量或数据库结构。
...
用户可见变化：
- 打开任务详情时，需求描述下方直接显示发布任务时上传的参考附件。
- 报酬和创建/发布时间仍在任务详情元信息区，但不再把附件夹在这些元信息之后。
- 如果详情远程读取刚好慢到超时，但页面已有本地任务内容，用户不再看到“内容已显示但顶部还有红色读取超时”的矛盾状态。
...
未验证内容：
- 未触发真实付款、真实采用投稿、真实结算或真实短信。
- 未在生产环境验证。
- 未用真实生产用户做跨账号权限回归。
- 未稳定复现一个“详情请求正好超时但本地任务已展示”的自动化场景，只按代码路径和本地慢请求日志验证兜底逻辑。
...
是否涉及权限：
- 不改变权限规则；附件预览和下载仍走原有访问路径。

是否涉及生产配置：
- 否。

是否已得到主人确认：
- 主人已明确要求调整任务详情页附件位置；超时红条兜底为同一任务详情体验问题下的前端展示修复，未改变业务规则或接口行为。

已知风险：
- 如果真实环境才虫详情或附件读取持续慢，用户仍可能在没有本地缓存可用时看到“读取超时”提示；本次只避免已有内容时残留红条。
- 任务详情页附件位置改变后，需要测试含多个附件、长需求描述、展开/收起需求、移动端窄屏下的布局。

建议测试重点：
- 含发布附件的任务详情：确认附件显示在需求描述下方，报酬和发布时间显示在附件之后。
- 长需求描述任务：确认“展开完整需求 / 收起需求”与附件位置不冲突。
- 多附件任务：确认附件换行、文件名省略、预览点击、键盘 Enter/Space 打开预览、下载按钮都正常。
- 慢详情读取场景：确认本地任务内容已显示时，不再残留顶部红色“读取超时”错误条。
- 移动端宽度：确认附件卡片和下载按钮不挤压、不遮挡。

## 最终上线总控结论 2026-06-05 任务详情附件层级与超时红条兜底

上线结论：可上线，带观察。

本轮版本：
- 分支：`main`
- 本轮起点：`5c0f873 Prepare current release package`
- 上线方式：提交本轮小修复并推送 `main`，由线上部署链路发布。

包含变更：
- 任务详情中，发布任务参考附件移动到需求描述正文下方。
- 附件区顶部外边距归零，让附件更像需求正文的一部分。
- 详情读取超时但已有本地任务内容可展示时，清掉顶部红色 `读取超时，请点击刷新重试` 错误条。
- 使用最新任务列表 ref 作为详情失败时的兜底来源，减少异步闭包里的旧任务列表影响。
...
- `npm run build` 通过。
- 本地 readiness 返回 `ready=true`。
- 本地任务详情接口和跨账号 404 权限边界已验证。
- 内置浏览器桌面首页和 390x844 窄屏首页冒烟通过，console error 为 0。
- 代码路径核对：附件块位置、键盘预览、下载阻止冒泡、超时红条兜底均符合本轮目标。

主要风险：
- 本地测试账号没有带发布附件任务，真实带附件任务详情仍需上线后人工补看。
- 未稳定复现真实“详情远程读取正好超过 8 秒但本地任务内容已展示”的浏览器场景；本轮按代码路径和慢请求背景验证。
- 移动端窄屏下的真实附件多行布局需上线后观察。
...
回滚建议：
- 如任务详情页面不可用，优先回滚本次提交。
- 如仅附件层级视觉不理想，可优先回滚 `components/order-console.tsx` 中附件块位置移动和 `app/globals.css` 中附件间距调整。
- 如超时红条兜底出现误清理真实错误，优先回滚 `DETAIL_FETCH_TIMEOUT_ERROR` / `tasksRef` 相关变更。
